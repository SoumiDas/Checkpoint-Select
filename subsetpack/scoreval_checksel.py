...
class DataValueCheckSel(object):
    ...
    def score_trajbatch(self):
        ...
        for ckpt in cps:
                ...
                for batch_idx, (inputs, targets, idx) in enumerate(trainsubloader):
                    inputs, targets = inputs.to(self.device), targets.to(self.device)
                    fv[ind] = self.get_feature(inputs)
                    ind = ind + 1
        ...
        return cpind,alphaval,fv,subset_trindices,cpv,testgr,res,dict_contrib,len(trainsubloader)

    ############ Computes values for datapoints not assigned to the selected trajectories(or out of batch)during training ###########

    def score_oobatch(self,cpind,alphaval,fv,subset_trindices,cpv,testgr,res,dict_contrib,lensub):
        ...
        trainrem = torch.utils.data.DataLoader(subset_rem, batch_size=1, shuffle=True, num_workers=2)

        #Computes scores for instance e in Z \ B by finding neighbour n from B
        for batch_idx, (inp, targ, idx) in enumerate(trainrem):
           #print('Instance :'+str(batch_idx))
           start = time.time()
           inp, targ = inp.to(self.device), targ.to(self.device)
           feari = self.get_feature(inp)
           feari = np.expand_dims(feari, axis = 0)  
           nind = self.findnearest(feari, fv) #Finds neighbour z for e
           acind = subset_trindices[nind]
           epbt = cpv[acind]
           contrib_vec = None
           for el in range(len(epbt)):
              ep,bt = epbt[el] #finding checkpoint to be used for computing score as per Eq. 8
              alpha = alphaval[cpind.index((ep,bt))]
              net = torch.load(self.rootpath+'trajp/epoch_'+str(ep)+'_batch_'+str(bt)+'.pth')
              test_gradb = testgr['epoch_'+str(ep)+'_batch_'+str(bt)+'.pth']
              train_grad_single = self.helperobj.get_grad(inp, targ, net).unsqueeze(0)
              tempf = test_gradb*train_grad_single
              temp = (tempf + 0.5*(tempf*tempf))/lensub
              res[rem_ind[batch_idx]] += alpha*(torch.sum(temp).item()) # score
              ...
              if dict_contrib[rem_ind[batch_idx]] is None:
                dict_contrib[rem_ind[batch_idx]] = {}
                dict_contrib[rem_ind[batch_idx]] = contrib_vec
              else:
                dict_contrib[rem_ind[batch_idx]] = contrib_vec

           res[rem_ind[batch_idx]] = res[rem_ind[batch_idx]]/len(epbt)
           dict_contrib[rem_ind[batch_idx]] = dict_contrib[rem_ind[batch_idx]]/len(epbt)
    ...
    def scorevalue(self):
        ...
        if not self.keeprem:
            ...
            cpind,alphaval,fv,subset_trindices,cpv,testgr,res,dict_contrib,lensub = self.score_trajbatch()
            scores,contrib = self.score_oobatch(cpind,alphaval,fv,subset_trindices,cpv,testgr,res,dict_contrib,lensub)
